...
func expandLabelSelectorRequirement(l []interface{}) []unversioned.LabelSelectorRequirement {
	...
	for i, n := range l {
		...
		obj[i] = unversioned.LabelSelectorRequirement{
			Key:      in["key"].(string),
			Operator: in["operator"].(unversioned.LabelSelectorOperator),
...
func expandPersistentVolumeClaimSpec(l []interface{}) v1.PersistentVolumeClaimSpec {
	if len(l) == 0 || l[0] == nil {
	...
	obj := v1.PersistentVolumeClaimSpec{
		...
		Resources:   expandResourceRequirements(in["resources"].([]interface{})),
	}
...
func expandResourceRequirements(l []interface{}) v1.ResourceRequirements {
	if len(l) == 0 || l[0] == nil {
	...
	if v, ok := in["limits"].(map[string]interface{}); ok && len(v) > 0 {
		obj.Limits, _ = expandMapToResourceList(v)
	}
	if v, ok := in["requests"].(map[string]interface{}); ok && len(v) > 0 {
		obj.Requests, _ = expandMapToResourceList(v)
	}
	return obj
}
